...
interface CampaignTypeEditProps {
    campaignType: CampaignType;
}

export default function CampaignTypeEdit({ campaignType }: CampaignTypeEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        is_active: campaignType.is_active ?? true,
        sort_order: campaignType.sort_order || 0,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/marketing/campaign-types/${campaignType.id}`);
    };

    return (
        <AppLayout>
            <Head title={`Edit ${campaignType.name}`} />

            <div className="p-6">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold">Edit Campaign Type</h1>
