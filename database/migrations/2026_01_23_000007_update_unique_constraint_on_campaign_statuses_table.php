...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $hasRidingCompanyId = Schema::hasColumn('campaign_statuses', 'riding_company_id');

        if ($hasRidingCompanyId) {
            // If riding_company_id exists, drop constraint on (riding_company_id, slug) if it exists
            if ($this->hasUniqueConstraint('campaign_statuses', ['riding_company_id', 'slug'])) {
                $constraintName = $this->getUniqueConstraintName('campaign_statuses', ['riding_company_id', 'slug']);
                if ($constraintName) {
                    try {
                        DB::statement("ALTER TABLE `campaign_statuses` DROP INDEX `{$constraintName}`");
                    } catch (\Exception $e) {
            ...
            if (!$this->hasUniqueConstraint('campaign_statuses', ['riding_company_id', 'name'])) {
                try {
            ...
            if ($this->hasUniqueConstraint('campaign_statuses', ['company_id', 'slug'])) {
                $constraintName = $this->getUniqueConstraintName('campaign_statuses', ['company_id', 'slug']);
                if ($constraintName) {
                    try {
                        DB::statement("ALTER TABLE `campaign_statuses` DROP INDEX `{$constraintName}`");
                    } catch (\Exception $e) {
            ...
            if (!$this->hasUniqueConstraint('campaign_statuses', ['company_id', 'name'])) {
                try {
                    Schema::table('campaign_statuses', function (Blueprint $table) {
    ...
    public function down(): void
    {
        ...
        if ($hasRidingCompanyId) {
            ...
            if ($this->hasUniqueConstraint('campaign_statuses', ['riding_company_id', 'name'])) {
                $constraintName = $this->getUniqueConstraintName('campaign_statuses', ['riding_company_id', 'name']);
                if ($constraintName) {
                    try {
                        DB::statement("ALTER TABLE `campaign_statuses` DROP INDEX `{$constraintName}`");
                    } catch (\Exception $e) {
            ...
            if ($this->hasUniqueConstraint('campaign_statuses', ['company_id', 'name'])) {
                $constraintName = $this->getUniqueConstraintName('campaign_statuses', ['company_id', 'name']);
                if ($constraintName) {
                    try {
                        DB::statement("ALTER TABLE `campaign_statuses` DROP INDEX `{$constraintName}`");
                    } catch (\Exception $e) {
